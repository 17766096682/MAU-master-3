...
from my_utils import Weighted_mse_mae
...
class Model(object):
    def __init__(self, configs):
        ...
        self.balance_mae_mse = Weighted_mse_mae(70)
    ...
    def train(self, data, mask, itr):
        ...
        #                              ground_truth[:, 1:])
        balance_loss = self.balance_mae_mse(next_frames, ground_truth[:, 1:])

        loss_gen = balance_loss
        loss_gen.backward()
        self.optimizer.step()
        ...
        return next_frames, balance_loss.detach().cpu().numpy(), balance_loss.detach().cpu().numpy()
